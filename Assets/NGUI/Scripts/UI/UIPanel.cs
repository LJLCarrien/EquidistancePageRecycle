...
public class UIPanel : UIRect
{
    ...
    /// 此面板是否将显示在面板工具中（对于动态创建的临时面板，将其设置为“false”）
    /// </summary>
    ...
    /// 默认情况下，非裁剪面板使用摄影机的边界，面板的位置无效。
    /// 如果您希望面板的位置实际上与锚点一起使用，请将此字段设置为“true”。
    /// </summary>
    ...
    /// 默认情况下，所有面板都通过递增来管理其绘制调用的渲染队列
    /// 以便以正确的顺序绘制几何体。 您可以更改此行为。
    /// </summary>
    ...
    /// 渲染面板使用的队列。 默认值“3000”相当于“透明”。
    /// 仅当'renderQueue'设置为“Automatic”以外的其他值时才使用此属性。
    /// </summary>
    ...
    /// 面板使用的排序层 - 在将NGUI与Unity的2D系统混合时使用。
    /// 供稿人：Benzino07： http://www.tasharen.com/forum/index.php?topic=6956.15
    /// </summary>
    ...
    [HideInInspector]
    [SerializeField]
    Texture2D mClipTexture = null;

    // Panel's alpha (affects the alpha of all widgets) 面板的透明度（影响所有小部件的alpha）
    [HideInInspector]
    [SerializeField]
    float mAlpha = 1f;

    // Clipping rectangle 裁剪矩形
    [HideInInspector]
    [SerializeField]
    ...
    [HideInInspector]
    [SerializeField]
    string mSortingLayerName = null;

    // Whether a full rebuild of geometry buffers is required 是否需要完全重建几何缓冲区
    bool mRebuild = false;
    bool mResized = false;
    ...
    /// 剪切区域偏移用于使剪切面板（滚动视图）有效移动成为可能。
    /// 通过将剪辑偏移调整一个值来移动滚动视图，并通过反向调整变换位置。
    /// 这使得不必重建几何体成为可能，从而大大提高了性能。
    /// </summary>
    ...
    /// 剪切位置（XY）和尺寸（ZW）。
    /// 请注意，您不应在运行时修改此属性以重新定位裁剪。 改为调整clipOffset。
    /// </summary>

    [System.Obsolete("Use 'finalClipRegion' or 'baseClipRegion' instead ")]//请改用“finalClipRegion”或“baseClipRegion”
    ...
    // Temporary variable to avoid GC allocation 临时变量以避免GC分配
    ...
    /// 面板剪切矩形的局部空间角。 顺序是左下角，左上角，右上角，右下角。
    /// </summary>
    ...
    /// 面板剪切矩形的世界空间角。 顺序是左下角，左上角，右上角，右下角。
    /// </summary>
    ...
    /// 在将面板的alpha考虑在内之后，Widget的最终alpha
    /// </summary>
    ...
    /// 如果可以，将指定的窗口小部件插入到现有的一个绘制调用中。
    /// 如果不能，并且需要新的绘图调用，则返回“null”
    /// 因为绘制调用创建是一个延迟操作。
    /// </summary>
